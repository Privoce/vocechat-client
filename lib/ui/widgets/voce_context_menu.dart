...
typedef VoceContextMenuBuilder = Widget Function(
  BuildContext context,
  Animation<double> animation,
);
...
///
/// When open, the [VoceContextMenu] shows the child, or the widget returned
...
class VoceContextMenu extends StatefulWidget {
  ...
  /// [VoceContextMenu] is closed, when it is in the middle of opening,
  ...
  /// passed to the builder, using values exposed by [VoceContextMenu],
  /// like [VoceContextMenu.kEndBoxShadow], to match the native iOS
  /// animation as close as desired.
  ///
  /// ** See code in examples/api/lib/cupertino/context_menu/cupertino_context_menu.1.dart **
  /// {@end-tool}
  final VoceContextMenuBuilder builder;
  ...
  /// The widget that can be "opened" with the [VoceContextMenu].
  ///
  /// When the [VoceContextMenu] is long-pressed, the menu will open and
  ...
  /// doesn't size itself.
  ///
  /// When the [VoceContextMenu] is "closed", this widget acts like a
  ...
  @override
  State<VoceContextMenu> createState() => _VoceContextMenuState();
}

class _VoceContextMenuState extends State<VoceContextMenu>
  ...
  final double _midpoint = VoceContextMenu.animationOpensAt / 2;
  ...
  void initState() {
    ...
    _openController = AnimationController(
      ...
      vsync: this,
      upperBound: VoceContextMenu.animationOpensAt,
    );
      ...
      builder: (BuildContext context, Animation<double> animation) {
        if (widget.child == null) {
          final Animation<double> localAnimation =
              Tween<double>(begin: VoceContextMenu.animationOpensAt, end: 1)
                  .animate(animation);
  ...
  void initState() {
    ...
    _boxDecoration = DecorationTween(
      ...
      CurvedAnimation(
        ...
        curve: Interval(0.0, VoceContextMenu.animationOpensAt),
      ),
    );
  }
...
class _ContextMenuRoute<T> extends PopupRoute<T> {
  ...
  _ContextMenuRoute({
    ...
    this.barrierLabel,
    VoceContextMenuBuilder? builder,
    // ImageFilter? filter
    super.filter,
  ...
  final VoceContextMenuBuilder? _builder;
  final GlobalKey _childGlobalKey = GlobalKey();
